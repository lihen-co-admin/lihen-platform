-- GAP-041 corrective migration 1
-- Resolve PL/pgSQL ambiguity between the RETURNS TABLE output column
-- `operation_key` and the table column used as the ON CONFLICT target.
...
'GAP-041 atomic idempotency hardening. Preserves Phase 8.5 actor ownership, expiry and preview-only semantics; claim is atomic via UNIQUE(operation_key) and ON CONFLICT ON CONSTRAINT.';
