create or replace function lihen_private.set_product_initial_cost_cutover(p_actor uuid, p_operation_key text, p_history_id uuid, p_product_id uuid, p_unit_cost numeric, p_effective_at timestamp with time zone, p_notes text)
...
begin
  ...
  if not exists(select 1 from public.products p where p.id=p_product_id) then raise exception using errcode='23503',message='LIHEN_PRODUCT_NOT_FOUND'; end if;
