-- F5-S1A/B — Style identity + taxonomy proposal dry-run.
-- READ ONLY: does not update products, brands, categories, visibility or images.
-- Proposals are intentionally non-canonical until explicit human/business approval.
...
begin
  ...
  with style_base as (
    select
      ...
      p.name,
      case
        when nullif(btrim(p.sku),'') is not null and upper(btrim(p.sku)) ~ '^ST-[0-9]+$' then true
        else false
      ...
      case
        ...
        when lower(p.name) ~ '\\m(pantaloneta|short)\\M' and lower(p.name) ~ '\\mhombre\\M' then 'ROPA_DEPORTIVA_HOMBRE'
        when lower(p.name) ~ '\\m(conjunto|falda|short|top)\\M' or lower(p.name) like '%push up%' then 'ROPA_DEPORTIVA_MUJER'
  ...
  max_style_number as (
    select coalesce(max((regexp_match(upper(btrim(p.sku)), '^ST-([0-9]+)$'))[1]::integer), 0) as n
    from public.products p
  ...
  proposals as (
    select
      s.*,
      case
        when not s.has_style_sku then row_number() over (
          order by
  ...
  select
    ...
    array_remove(array[
      ...
      case when p.category_key = 'REVIEW_REQUIRED' then 'CATEGORY_REQUIRES_MANUAL_CLASSIFICATION' else 'CATEGORY_LABEL_PROPOSAL_ONLY' end,
...
comment on function public.get_style_identity_taxonomy_dry_run_controlled() is
  'F5-S1A/B read-only proposal matrix for Style identity and taxonomy. No proposal is canonical and no product/category/brand is mutated.';
