create or replace function public.get_storefront_products_qa_a_controlled(
...
revoke all on function public.get_storefront_products_qa_a_controlled(integer,integer,text,text,text,text,text) from public;
grant execute on function public.get_storefront_products_qa_a_controlled(integer,integer,text,text,text,text,text) to anon,authenticated;
comment on function public.get_storefront_products_qa_a_controlled(integer,integer,text,text,text,text,text) is 'F5 QA-A read-only storefront projection. Adds the CARE navigation preset while preserving canonical publication, media and availability rules.';
