...
create table if not exists lihen_private.catalog_pdf_image_storage_operations (
  ...
  constraint catalog_pdf_image_operation_key_not_blank
    check (length(btrim(operation_key)) > 0),
  constraint catalog_pdf_image_fingerprint_check
    check (request_fingerprint ~ '^[0-9a-f]{32}$'),
  constraint catalog_pdf_image_bucket_check
    check (bucket_id = 'lihen-product-web'),
  constraint catalog_pdf_image_product_image_unique
    unique (product_image_id),
  constraint catalog_pdf_image_bucket_path_unique
    unique (bucket_id, object_path)
);

revoke all on table lihen_private.catalog_pdf_image_storage_operations
from public, anon, authenticated;

grant select, insert
on table lihen_private.catalog_pdf_image_storage_operations
to service_role;


create or replace function public.finalize_catalog_pdf_image_storage(
...
declare
  ...
  v_existing_op lihen_private.catalog_pdf_image_storage_operations%rowtype;
...
begin
  if p_operation_key is null or length(btrim(p_operation_key)) = 0 then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_OPERATION_KEY_REQUIRED';
  end if;

  if p_product_image_id is null or p_product_id is null or p_source_id is null then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_ID_REQUIRED';
  end if;

  if p_bucket_id <> 'lihen-product-web' then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_BUCKET_INVALID';
  end if;

  if p_mime_type <> 'image/webp' then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_MIME_INVALID';
  end if;

  if p_byte_size is null or p_byte_size <= 0 or p_byte_size > 3145728 then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_SIZE_INVALID';
  end if;

  if p_sha256 is null or p_sha256 !~ '^[0-9a-f]{64}$' then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_SHA256_INVALID';
  end if;

  if p_width_px is null or p_width_px <= 0
     or p_height_px is null or p_height_px <= 0 then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_DIMENSIONS_INVALID';
  end if;

  v_expected_path :=
    'products/' || p_product_id::text || '/' ||
    p_product_image_id::text || '/web/' ||
    p_sha256 || '.webp';

  if p_object_path is distinct from v_expected_path then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_PATH_INVALID';
  end if;

  if p_public_url is null
     or p_public_url not like
       '%/storage/v1/object/public/lihen-product-web/' || v_expected_path then
    raise exception using errcode='22023',
      message='LIHEN_CATALOG_PDF_IMAGE_PUBLIC_URL_INVALID';
  end if;
  ...
  if not found then
    raise exception using errcode='P0002',
      message='LIHEN_CATALOG_PDF_IMAGE_PROVENANCE_NOT_ELIGIBLE';
  end if;
  ...
  if not found then
    raise exception using errcode='P0002',
      message='LIHEN_PRODUCT_NOT_FOUND';
  end if;

  v_fingerprint := md5(concat_ws('|',
    p_product_image_id::text,
    p_product_id::text,
    p_source_id::text,
    p_bucket_id,
    p_object_path,
  ...
  select o.*
  into v_existing_op
  from lihen_private.catalog_pdf_image_storage_operations o
  ...
  if found then
    if v_existing_image.product_id <> p_product_id
       ...
       or v_existing_image.source_id is distinct from p_source_id
       or v_existing_image.asset_role <> 'DERIVATIVE'
       or v_existing_image.derivative_profile is distinct from 'CATALOG_PDF'
       or v_existing_image.status <> 'ACTIVE' then
      raise exception using errcode='23505',
        message='LIHEN_CATALOG_PDF_IMAGE_METADATA_CONFLICT';
    end if;
  else
  ...
  if found then
    if v_existing_asset.product_id <> p_product_id
       ...
       or v_existing_asset.rendition_profile is distinct from 'CATALOG_PDF' then
      raise exception using errcode='23505',
        message='LIHEN_CATALOG_PDF_IMAGE_STORAGE_ASSET_CONFLICT';
    end if;
  ...
  insert into lihen_private.catalog_pdf_image_storage_operations(
    operation_key,
    request_fingerprint,
    product_id,
    product_image_id,
  ...
  values (
    ...
    p_object_path,
    jsonb_build_object(
      'product_image_id', p_product_image_id,
      'product_id', p_product_id,
      'source_id', p_source_id,
      'storage_asset_id', v_asset_id,
      'storage_bucket', p_bucket_id,
...
revoke all on function public.finalize_catalog_pdf_image_storage(
  text,uuid,uuid,uuid,text,text,text,text,bigint,text,integer,integer
) from public, anon, authenticated;

grant execute on function public.finalize_catalog_pdf_image_storage(
  text,uuid,uuid,uuid,text,text,text,text,bigint,text,integer,integer
) to service_role;

comment on function public.finalize_catalog_pdf_image_storage(
  text,uuid,uuid,uuid,text,text,text,text,bigint,text,integer,integer
) is
'Idempotent metadata finalizer for prepared CATALOG_PDF product-image derivatives. Does not replace WEB_DETAIL, does not mutate products.main_image_url and does not activate catalog versions.';
